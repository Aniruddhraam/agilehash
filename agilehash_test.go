...
func TestHash(t *testing.T) {
	for _, tc := range testVectors {
		got := agilehash.Hash([]byte(tc.input))
		if got != tc.expected {
...
func TestHashEmpty(t *testing.T) {
	hash := agilehash.Hash(nil)
	hashEmpty := agilehash.Hash([]byte{})
...
func TestHashWithSeed(t *testing.T) {
	...
	hash0 := agilehash.HashWithSeed(data, 0)
	hash1 := agilehash.HashWithSeed(data, 1)
	hash2 := agilehash.HashWithSeed(data, 12345)
...
func TestHashAndMicroMatchUpTo80Bytes(t *testing.T) {
	...
	for i := 0; i <= 80; i++ {
		...
		hashMicro := agilehash.HashMicro(data)
...
func TestHashMicroWithSeed(t *testing.T) {
	...
	// Different seeds should produce different hashes
	hash0 := agilehash.HashMicroWithSeed(data, 0)
	hash1 := agilehash.HashMicroWithSeed(data, 1)
	hash2 := agilehash.HashMicroWithSeed(data, 12345)
...
func TestHashMicroWithSeedAllSizes(t *testing.T) {
	...
	for _, size := range sizes {
		...
		hash := agilehash.HashMicroWithSeed(data, seed)
		hashNoSeed := agilehash.HashMicro(data)
		...
		hash2 := agilehash.HashMicroWithSeed(data, seed)
		if hash != hash2 {
...
func TestHashMicroWithSeedEmpty(t *testing.T) {
	...
	hash1 := agilehash.HashMicroWithSeed(nil, 0)
	hash2 := agilehash.HashMicroWithSeed([]byte{}, 0)
	...
	hash3 := agilehash.HashMicroWithSeed(nil, 12345)
	if hash1 == hash3 {
...
func TestHashNanoWithSeed(t *testing.T) {
	...
	// Different seeds should produce different hashes
	hash0 := agilehash.HashNanoWithSeed(data, 0)
	hash1 := agilehash.HashNanoWithSeed(data, 1)
	hash2 := agilehash.HashNanoWithSeed(data, 12345)
...
func TestHashNanoWithSeedAllSizes(t *testing.T) {
	...
	for _, size := range sizes {
		...
		// Should not panic and should produce non-zero hash (except possibly for empty)
		hash := agilehash.HashNanoWithSeed(data, seed)
		hashNoSeed := agilehash.HashNano(data)
		...
		// Consistency check: same input should produce same output
		hash2 := agilehash.HashNanoWithSeed(data, seed)
		if hash != hash2 {
			t.Errorf("size=%d: inconsistent results 0x%x vs 0x%x", size, hash, hash2)
		}
	}
...
func TestHashNanoWithSeedEmpty(t *testing.T) {
	// Test empty input path specifically
	hash1 := agilehash.HashNanoWithSeed(nil, 0)
	hash2 := agilehash.HashNanoWithSeed([]byte{}, 0)
	...
	// Empty with different seeds should differ
	hash3 := agilehash.HashNanoWithSeed(nil, 12345)
	if hash1 == hash3 {
		t.Errorf("Empty input with different seeds should produce different hashes")
	}
}
...
func TestAllVariantsAgainstCReference(t *testing.T) {
	for _, tc := range sizeTestVectors {
		...
		gotHash := agilehash.Hash(data)
		gotMicro := agilehash.HashMicro(data)
		gotNano := agilehash.HashNano(data)
...
func TestAllVariantsWithSeedMatchForSmallInputs(t *testing.T) {
	...
	for i := 0; i <= 48; i++ {
		...
		hash := agilehash.HashWithSeed(data, seed)
		hashMicro := agilehash.HashMicroWithSeed(data, seed)
		hashNano := agilehash.HashNanoWithSeed(data, seed)
...
func TestVariantsMatchForSmallInputs(t *testing.T) {
	...
	for i := 0; i <= 48; i++ {
		...
		hash := agilehash.Hash(data)
		hashMicro := agilehash.HashMicro(data)
		hashNano := agilehash.HashNano(data)
...
func TestLengthVariation(t *testing.T) {
	...
	ab := agilehash.Hash([]byte("ab"))
	a := agilehash.Hash([]byte("a"))
	b := agilehash.Hash([]byte("b"))
...
func TestHashStringMatchesHash(t *testing.T) {
	...
	for _, s := range testStrings {
		expected := agilehash.Hash([]byte(s))
		got := agilehash.HashString(s)
		if got != expected {
...
func TestHashStringWithSeedMatchesHashWithSeed(t *testing.T) {
	...
	for _, s := range testStrings {
		for _, seed := range seeds {
			expected := agilehash.HashWithSeed([]byte(s), seed)
			got := agilehash.HashStringWithSeed(s, seed)
			if got != expected {
...
func TestHashStringNanoMatchesHashNano(t *testing.T) {
	...
	for _, s := range testStrings {
		expected := agilehash.HashNano([]byte(s))
		got := agilehash.HashStringNano(s)
		if got != expected {
...
func TestHashStringNanoWithSeedMatchesHashNanoWithSeed(t *testing.T) {
	...
	for _, s := range testStrings {
		for _, seed := range seeds {
			expected := agilehash.HashNanoWithSeed([]byte(s), seed)
			got := agilehash.HashStringNanoWithSeed(s, seed)
			if got != expected {
...
func TestHashStringMicroMatchesHashMicro(t *testing.T) {
	...
	for _, s := range testStrings {
		expected := agilehash.HashMicro([]byte(s))
		got := agilehash.HashStringMicro(s)
		if got != expected {
...
func TestHashStringMicroWithSeedMatchesHashMicroWithSeed(t *testing.T) {
	...
	for _, s := range testStrings {
		for _, seed := range seeds {
			expected := agilehash.HashMicroWithSeed([]byte(s), seed)
			got := agilehash.HashStringMicroWithSeed(s, seed)
			if got != expected {
...
func TestHashStringAllSizes(t *testing.T) {
	...
	for _, size := range sizes {
		...
		// Test all variants
		if got, want := agilehash.HashString(s), agilehash.Hash(data); got != want {
			t.Errorf("size %d: HashString = 0x%x, want 0x%x", size, got, want)
		}
		if got, want := agilehash.HashStringNano(s), agilehash.HashNano(data); got != want {
			t.Errorf("size %d: HashStringNano = 0x%x, want 0x%x", size, got, want)
		}
		if got, want := agilehash.HashStringMicro(s), agilehash.HashMicro(data); got != want {
			t.Errorf("size %d: HashStringMicro = 0x%x, want 0x%x", size, got, want)
		}

		// Test with seed
		seed := uint64(12345)
		if got, want := agilehash.HashStringWithSeed(s, seed), agilehash.HashWithSeed(data, seed); got != want {
			t.Errorf("size %d: HashStringWithSeed = 0x%x, want 0x%x", size, got, want)
		}
		if got, want := agilehash.HashStringNanoWithSeed(s, seed), agilehash.HashNanoWithSeed(data, seed); got != want {
			t.Errorf("size %d: HashStringNanoWithSeed = 0x%x, want 0x%x", size, got, want)
		}
		if got, want := agilehash.HashStringMicroWithSeed(s, seed), agilehash.HashMicroWithSeed(data, seed); got != want {
