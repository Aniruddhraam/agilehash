...
func TestHashComparableNaN(t *testing.T) {
	...
	h1 := agilehash.HashComparableWithSeed(v, seed)
	h2 := agilehash.HashComparableWithSeed(v, seed)
...
func TestWriteComparable(t *testing.T) {
	...
	h := agilehash.NewWithSeed(seed)
	h.WriteComparable(v)

	encoded := encodeComparableTest(v)
	expected := agilehash.HashWithSeed(encoded, seed)
...
func TestWriteComparableWithOtherWrites(t *testing.T) {
	...
	expected := agilehash.HashWithSeed(combined, seed)
...
func hashComparableAny(t *testing.T, v any) uint64 {
	...
	switch x := v.(type) {
	case int:
		return agilehash.HashComparable(x)
	case uint64:
		return agilehash.HashComparable(x)
	case string:
		return agilehash.HashComparable(x)
