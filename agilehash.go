...
//
//go:nosplit
func loadUpTo3(p unsafe.Pointer, length int) (uint64, uint64) {
	return (uint64(*(*byte)(p)) << 45) | uint64(*(*byte)(add(p, uintptr(length-1)))),
		uint64(*(*byte)(add(p, uintptr(length>>1))))
}
...
// Hash computes a 64-bit agilehash of the input data using the default seed (0).
...
// HashWithSeed computes a 64-bit agilehash of the input data using the provided
// seed.
...
// HashString computes a 64-bit agilehash of the input string using the default
// seed (0).
func HashString(s string) uint64 {
	return Hash(stringToBytes(s))
}

// HashStringWithSeed computes a 64-bit agilehash of the input string using the
// provided seed.
