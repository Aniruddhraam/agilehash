...
func TestHasherWithSeed(t *testing.T) {
	...
	h1 := agilehash.NewWithSeed(12345)
	_, _ = h1.Write(data)
	...
	if h0.Sum64() != agilehash.Hash(data) {
		t.Error("New() should match Hash()")
	}

	if h1.Sum64() != agilehash.HashWithSeed(data, 12345) {
...
func TestHasherImplementsHashHash64(t *testing.T) {
	var _ hash.Hash64 = agilehash.New()
}
...
func TestHasherWriteString(t *testing.T) {
	...
	for _, s := range testStrings {
		...
		if h2.Sum64() != agilehash.HashString(s) {
			t.Errorf("WriteString(%q) = 0x%x, HashString = 0x%x", s, h2.Sum64(), agilehash.HashString(s))
		}
	}
}
...
func TestHasherWriteStringChunked(t *testing.T) {
	...
	expected := agilehash.HashString(s)
